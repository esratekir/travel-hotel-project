  ...
  <div class="container-fluid">
    <div class="row">
      <div class="col-12">
        <div class="card">
          <div class="card-body">
            <h4 class="card-title">Rehber Resimleri Ekleme</h4><br>
            <form method="post" action="{{ route('store.images')}}" enctype="multipart/form-data" class="needs-validation" novalidate>   <!--buradaki enctype veritabanına resim yüklenmesine yarıyor -->
              @csrf
              <div class="row mb-3">
                ...
                <div class="col-sm-10">
                  <select name="guide" class="form-select" required>
                    ...
                    @foreach($users as $user)
                      <option value="{{ $user->user_id }}" {{ $user->user_id == $user_name->user_id ? 'selected' : ''}}>{{ $user->name }}</option>
                    @endforeach
                  </select>
                </div>
              </div> <!--end row-->
