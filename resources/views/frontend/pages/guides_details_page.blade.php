        ...
        <div class="d-flex flex-column text-center bg-white mb-4 py-4 px-4">
          <div id="header-carousel" class="carousel slide" data-ride="carousel">
            <div class="carousel-inner">
              @php($a = 0)
              @foreach($guide_images as $item)
                @php($a++)
                <div class="carousel-item {{ $a == 1 ? 'active' : '' }}">
                  <img src="{{asset($item->image)}}" class="img-fluid mx-auto mb-3" style="margin-top:34px;height:300px;" alt="guide_images">
                </div>
              @endforeach
            </div>
  
            @if(count($guide_images) > 1)
              <a class="carousel-control-prev" href="#header-carousel" data-slide="prev">
                <div class="btn btn-dark" style="width: 40px; height: 40px;">
          ...
          <h3 class="text-primary mb-3">{{$guides->name}}</h3>
          <small class="m-0"><i class="fa fa-map-marker-alt text-primary mr-2"></i>{{$guides['citi']['name']}} / {{$guides['Country']['country']}}</small><br>       
          <p class="para"> " {{ $guides->motto }} " </p>              
          <div class="d-flex justify-content-center">
            <h5 class="m-0">${{$guides->price}}<small style="font-size: 13px;">/h</small></h5>
          </div>
        </div>
        <div class="rezervasyon-wrap d-flex flex-column text-center bg-white br-5 mb-4 py-4 px-4">
          <h3 class="text-danger mb-3">Make A Reservation</h3>
          <p>You can make a reservation or send message about guides via WhatsApp.</p>
          <div class="d-flex justify-content-center">
            @if(Auth::check())
            <div class="call-to" style="width: 100%;">
              <a type="button" class="btn buttons btn-block" href="{{route('my.messages.details', $guides->user_id)}}"> CONTACT</a>
            </div>
            @else
            <div class="call-to" style="width:100%;">
              <a type="button" class="btn buttons btn-block" data-toggle="modal" data-target="#girisModal"> CONTACT</a>
            </div>
            @endif
          </div>
        </div>
      ...
      <div class="col-lg-8">
            ...
            <div class="row">
              <div class="bg-white col-md-12" style="padding: 30px;">
                <div class="pl-md-10 pr-md-8 py-7 titl">
                  <h2 class="fs-30 text-dark font-weight-600 lh-16 mb-0">Explore {{$guides['citi']['name']}} , {{$guides['Country']['country']}} , with  {{$guides->name}}</h2><br>
                  <p class="mb-0 m">About Me</p><hr>
                  <p class="content">{!! substr($guides->description, 0, 200) !!}</p>
                  <a href="#" class="read-more-link" data-full-content="{!! $guides->description !!}">+More</a>
                  <hr class="mb-4">
                  <div class="row titl">
                    <div class="col-sm-6 mb-4">
                      <p class="mb-0 m">Activities</p><hr>
                      @foreach($guides->Activit as $act)
                      <p class="text-heading font-weight-500 mb-0 lh-13">
                        <i class="{{$act->icon}}" aria-hidden="true"></i>  {{$act->activity_name}}
                      </p>
                      @endforeach
                    </div>
                    <div class="col-sm-6 mb-4">
                      <p class="mb-0 m">Language</p><hr>
                      @foreach($guides->Language as $lang)
                      <p class="text-heading font-weight-500 mb-0 lh-13">
                        <img class="mr-2" src="{{asset($lang->flag)}}" alt="language_flag">  {{$lang->language}}
                      </p>
                      @endforeach
                    </div>
                  </div>
                  ...
                  <div class="row align-items-center">
                    <div class="col-sm-6 mb-6 mb-sm-0">
                      <ul class="list-inline mb-0">
                        ...
                        <li class="list-inline-item fs-13 text-gray-light">{{count($guides->comments)}} Reviews</li>
                      </ul>
                    </div>
                  </div>
                </div>
        ...
        <div class="bg-white mb-3" style="padding: 30px;">
          ...
          <form method="post" action="{{route('store.comments')}}">
            @csrf
            <input type="hidden" name="guide_id" value="{{$guides->user_id}}">
        ...
        <div class="bg-white" style="padding: 30px; margin-bottom: 30px;">
          <h4 class="text-uppercase mb-4" style="letter-spacing: 5px;">{{count($guides->comments)}} Comments</h4>           
          @foreach($guides->comments as $comme)           
            <div class="media mb-4" >
              @if($comme->user)
                ...
                <div class="media-body">
                  <h6><a href="{{ route('local.user.details', $comme->user->username) }}">{{$comme->user->name}}</a> <small><i>{{$comme->created_at->diffForHumans()}}</i></small></h6>
                  <p>{!! $comme->comment !!}</p>
                  @if(Auth::check())
                  @endif
                  @endif                
        ...
        @if(Auth::check())
          <div class="bg-white mb-3 replyDiv" style="padding: 30px; display:none;">
            ...
            <form method="post" action="{{route('store.comments')}}">
              @csrf
              <input type="hidden" name="guide_id" value="{{$guides->user_id}}">
              @foreach($guides->comments as $com)
            ...
            <div class="MembershipPricing MembershipPricing--Desktop row">
              <div class="col-md-4">
                <div class="box" >
                  ...
                  <div class="price">
                    <span  class="ng-binding">€0.00</span>/mo.
                  </div>
                  <div class="text billed ng-binding">
                    €0.00 for 1 month
                  </div>
                  <a class="btn Button--membership simple ng-isolate-scope" loader-class="Loader--blue Loader--small" href="{{route('iyzipay.pay')}}">
                    Subscribe
                  </a>
              ...
              <div class="col-md-4">
                <div class="box" >
                  ...
                  <div class="price">
                    <span  class="ng-binding">€40.00</span>/mo.
                  </div>
                  <div class="text billed ng-binding">
                    €119.99 for 3 months
                  </div>
                  <a class="btn Button--membership simple ng-isolate-scope" loader-class="Loader--blue Loader--small" href="{{ route('home')}}">
                    Subscribe
                  </a>
                </div>
              </div>
              <div class="col-md-4">
                ...
                <div class="price">
                  <span  class="ng-binding">€29.17</span>/mo.
                </div>
                <div class="text billed ng-binding">
                  €349.99 for one year
                </div>
...
<div class="modal fade" id="odemeModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-body">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              ...
              <form method="post" action="" enctype="multipart/form-data">
                ...
                <div class="form-row">
                  <div class="control-group col-lg-12">
                    ...
                    <input type="text" name="name" class="form-control mb-3"  required="required" placeholder="Cardholder Name"/>
                  </div>
                </div>
                <div class="form-row">
                  <div class="control-group col-lg-12">
                    <label>Card Number:</label>
                    <input type="email" name="card_number" class="form-control mb-3" placeholder=".... .... .... ...."/>
                  </div>
                </div>
                <div class="form-row mb-3">
                  <div class="control-group col-sm-6">
                    <label>Expiration Date:</label>
                    <input type="tel" name="expi_date" class="form-control" placeholder="MM/YY"/>
                  </div>
                  <div class="control-group col-sm-6">
                    <label>CVC:</label>
                    <input type="tel" name="cvc" class="form-control" placeholder="..."/>
                  </div>
                </div>

                <div class="row mt-2">
                  <div class="col-md-12">
                    <button type="submit" class="btn btn-primary margin-top-0 margin-bottom-20">Update</button>
                  </div>
                </div>
              </form>
            </div>
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    readMoreLinks.forEach(link => {
      link.addEventListener("click", function (e) {
        ...
        const contentParagraph = this.previousElementSibling;
        contentParagraph.textContent = fullContent;
